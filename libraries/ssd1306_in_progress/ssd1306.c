...
P0.5--------------------SDA
RESET--------------------------------------DTR
			SCL--10K Resistor--3.3v
			SDA--10K Resistor--3.3V
*/
...
void LED_WrCmd( unsigned char data) {
	uint8_t control = 0x00;   // Co = 0, D/C = 0
	w2_master_write_to(LCD_ADDRESS, &control, 1, &data, 1);
}
...
void LED_Fill(unsigned char bmp_data)
{
	...
	for(y=0;y<8;y++)
	{
		...
		LED_WrCmd(0x00);     
		LED_WrCmd(0x10);	     
		w2_master_write_to(LCD_ADDRESS, &control, 1, row, 128); 
	}
} 
...
void LED_P6x8Str(unsigned char x,unsigned char y,char ch[])
{
	...
	{    
		...
		w2_master_write_to(LCD_ADDRESS, &control, 1, sixBytes, 6); 
		x+=6;
		j++;
	}
}
...
void LED_P8x16Str(unsigned char x,unsigned char y,char ch[])
{
	...
	{    
		...
		for(i=0;i<8;i++) 
		{
			eightBytes[i]= F8X16[(c<<4)+i];
		}

		w2_master_write_to(LCD_ADDRESS, &control, 1, eightBytes, 8); 

		LED_Set_Pos(x,y+1);    
		for(i=0;i<8;i++) 
		{
			eightBytes[i]= F8X16[(c<<4)+i+8];
		}
		w2_master_write_to(LCD_ADDRESS, &control, 1, eightBytes, 8); 
		x+=8;
		j++;
